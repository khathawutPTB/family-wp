...
- Database: SQLite ใน local dev (ไฟล์เดียว ไม่ต้องตั้งรหัสผ่าน) — สลับไปใช้ PostgreSQL ได้ตอน deploy จริง ดู [สลับไปใช้ PostgreSQL](#สลับไปใช้-postgresql-ภายหลัง)

> โปรเจกต์นี้เริ่มออกแบบไว้ให้ใช้ PostgreSQL แต่ตอน setup ในเครื่องนี้เข้าถึงรหัสผ่าน PostgreSQL ที่ติดตั้งไว้ไม่ได้ (ไม่ใช่ `postgres/postgres`) จึงสลับ `prisma/schema.prisma` มาใช้ SQLite ชั่วคราวเพื่อให้รันระบบได้ทันที ข้อมูลกับ schema ทั้งหมดเหมือนเดิมทุกอย่าง ต่างกันแค่ตัว database engine
...
├── type                      -- "INCOME" | "EXPENSE" (String — validate ที่ API layer เพราะ SQLite ไม่มี native enum)
├── icon                      -- emoji
...
├── member_id (FK -> family_members.id, nullable)  -- ไม่บังคับแท็กสมาชิก
├── type                      -- "INCOME" | "EXPENSE"
├── amount                    -- Decimal
├── date
├── note
...
3. **ฟอร์มบันทึกรายการ** — เลือกรายรับ/รายจ่าย, จำนวนเงิน, วันที่, หมวดหมู่, สมาชิก (ไม่บังคับ), หมายเหตุ (ใช้ฟอร์มเดียวกันทั้งเพิ่มและแก้ไข)
...
โปรเจกต์นี้ตั้งค่าไว้ให้รันได้ทันทีด้วย SQLite (ทำและทดสอบให้เรียบร้อยแล้ว) ไม่ต้องสร้างฐานข้อมูลหรือตั้งรหัสผ่านใดๆ ก่อน
...
ไฟล์ `backend/.env` มีอยู่แล้ว (ค่าเริ่มต้นใช้ SQLite) แต่ถ้าต้องสร้างใหม่:
...
รัน migration เพื่อสร้างไฟล์ฐานข้อมูล SQLite ตาม schema และ seed หมวดหมู่เริ่มต้น (ทำไปแล้วครั้งหนึ่ง แต่ถ้าลบ `prisma/dev.db` ทิ้งก็รันซ้ำได้):
...
**ข้อจำกัด:** วิธีนี้ใช้ได้แค่ตอนคอมพิวเตอร์เปิดเครื่อง รัน `npm run dev` ทั้ง backend/frontend อยู่ และมือถือต่อ Wi-Fi วงเดียวกันเท่านั้น ถ้าต้องการเข้าถึงจากอินเทอร์เน็ตทั่วไป (นอกวง Wi-Fi) หรือใช้งานถาวร ต้อง deploy frontend/backend/database ขึ้น hosting จริง (เช่น Vercel/Netlify สำหรับ frontend, Railway/Render สำหรับ backend + PostgreSQL)

## สลับไปใช้ PostgreSQL ภายหลัง

เมื่อทราบ/รีเซ็ตรหัสผ่านของ PostgreSQL ในเครื่องแล้ว (เปิด pgAdmin 4 ที่ติดตั้งมาด้วยกันเพื่อดู/เปลี่ยนรหัสผ่าน หรือรีเซ็ตผ่าน `pg_hba.conf` เอง) ทำตามนี้:

1. สร้างฐานข้อมูลเปล่า:
   ```bash
   psql -U postgres -c "CREATE DATABASE expense_tracker;"
   ```
2. แก้ `backend/prisma/schema.prisma` — เปลี่ยน `datasource db { provider = "sqlite" ... }` กลับเป็น `provider = "postgresql"`
3. เพิ่ม native type กลับเข้าไปที่ `Transaction.amount` และ `Transaction.date` (ถูกถอดออกตอนสลับมา SQLite เพราะ SQLite ไม่รองรับ):
   ```prisma
   amount  Decimal  @db.Decimal(12, 2)
   date    DateTime @db.Date
   ```
4. (ตัวเลือก) เปลี่ยน `type String` กลับเป็น `enum TransactionType { INCOME EXPENSE }` แล้วใช้ type นั้นแทน String ในทั้งสองโมเดล เพื่อให้ Postgres บังคับ constraint ระดับ DB ด้วย
5. แก้ `backend/.env`:
   ```
   DATABASE_URL="postgresql://<user>:<password>@localhost:5432/expense_tracker"
   ```
6. ลบไฟล์ `backend/prisma/dev.db` และโฟลเดอร์ `backend/prisma/migrations` ทิ้ง แล้วรัน migration ใหม่:
   ```bash
   npx prisma migrate dev --name init
   node prisma/seed.js
   ```
